...
Shader "DX11/3D Texture Raymarching/Bucky/MediumDetail"
{
	Properties
	{
		_Volume("Texture", 3D) = "" {}
		...
		_SkullCentre("Skull Centre", Vector) = (0,0,0,0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				if (p.x > _SkullCentre.x + _BoxWidth / 2)
					return false;
				if (p.x < _SkullCentre.x - _BoxWidth / 2)
					return false;
				if (p.y > _SkullCentre.y + _BoxLength / 2)
					return false;
				if (p.y < _SkullCentre.y - _BoxLength / 2)
					return false;
				if (p.z > _SkullCentre.z + _BoxHeight / 2)
					return false;
				if (p.z < _SkullCentre.z - _BoxHeight / 2)
					return false;
			...
			//return lerp(fixed4(1, 0, 0, 1), fixed4(0, 1, 0, 1), step(_Radius, distance(position, _SkullCentre)) );
			//return fixed4(frac(distance(position, _SkullCentre.xyz)), 0,0, 1);
